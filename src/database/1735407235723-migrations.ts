...
export class Migrations1735407235723 implements MigrationInterface {
  name = 'Migrations1735407235723';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`attendance\` (\`id\` varchar(36) NOT NULL, \`timestamp\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`status\` enum ('present', 'absent', 'late') NOT NULL DEFAULT 'absent', \`student_id\` varchar(36) NOT NULL, \`course_id\` varchar(36) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
